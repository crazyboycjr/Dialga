 ...
 *  \file sarray.h
 *  \brief this file is extracted from ps-lite's sarray.h, I make some
 *  modifications to it, so that it can adapt to use defined allocator
 */
#ifndef DIALGA_SARRAY_HPP_
#define DIALGA_SARRAY_HPP_
#include <string.h>
#include <memory>
#include <sstream>
#include <string>
...
#include "internal/alloc/allocator.hpp"
 ...
 */
template <typename V, typename Allocator = alloc::DefaultAllocator<V>>
class SArray {
 public:
  ...
  explicit SArray(const SArray<W, Alloc>& arr) {
    *this = arr;
  }
   ...
   */
  template <typename W, typename Alloc>
  void operator=(const SArray<W, Alloc>& arr) {
  ...
  void resize(size_t size, V val = 0) {
    ...
    if (capacity_ >= size) {
      ...
      V* new_data = Allocator::Alloc(size + 5);
      memcpy(new_data, data(), size_ * sizeof(V));
      reset(new_data, size, [](V* data) { Allocator::Free(data); });
    }
  ...
  inline size_t bytes() const { return size_ * sizeof(V); }
